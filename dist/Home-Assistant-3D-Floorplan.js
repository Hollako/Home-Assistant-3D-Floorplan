...
class HomeAssistant3DFloorplan extends HTMLElement {
  ...
  _opaqueThreeColorValue(value, fallback = "#111827") {
    const color = String(value || "").trim() || fallback;
    const commaRgba = color.match(/^rgba\(\s*([^,]+)\s*,\s*([^,]+)\s*,\s*([^,]+)\s*,\s*[^)]+\)$/i);
    if (commaRgba) return `rgb(${commaRgba[1]}, ${commaRgba[2]}, ${commaRgba[3]})`;
    const modernRgba = color.match(/^rgb\(\s*([^/]+?)\s*\/\s*[^)]+\)$/i);
    if (modernRgba) return `rgb(${modernRgba[1].trim()})`;
    const commaHsla = color.match(/^hsla\(\s*([^,]+)\s*,\s*([^,]+)\s*,\s*([^,]+)\s*,\s*[^)]+\)$/i);
    if (commaHsla) return `hsl(${commaHsla[1]}, ${commaHsla[2]}, ${commaHsla[3]})`;
    const modernHsla = color.match(/^hsl\(\s*([^/]+?)\s*\/\s*[^)]+\)$/i);
    if (modernHsla) return `hsl(${modernHsla[1].trim()})`;
    return color;
  }

  _modelObjectConfigs(value) {
    if (!Array.isArray(value)) return [];
    return value.filter((item) => item && typeof item === "object" && !Array.isArray(item) && String(item.object_name || "").trim());
  }
  ...
  async _renderModelViewer(modelUrl) {
    ...
    try {
      ...
      scene.background = new THREE.Color(this._opaqueThreeColorValue(background));
      ...
      const animationConfigs = this._modelObjectConfigs(activeFloorForAnim.animations || this._config.animations);
      ...
      const interactiveConfigs = this._modelObjectConfigs(activeFloorForAnim.interactive_objects || this._config.interactive_objects);
...
class HomeAssistant3DFloorplanEditor extends HTMLElement {
  ...
  _renderObjectConfigToolbar(kind) {
    ...
    const scopeOptions = [
      ...
      ...floors.map((floor, index) => {
        const floorConfig = floor && typeof floor === "object" && !Array.isArray(floor) ? floor : {};
        const value = `floor:${index}`;
        const label = floorConfig.name || floorConfig.id || `Floor ${index + 1}`;
  ...
  _objectConfigTarget(config = this._config) {
    ...
    const floor = Array.isArray(config.floors) ? config.floors[index] : null;
    return floor && typeof floor === "object" && !Array.isArray(floor) ? floor : config;
  }
  ...
  _renderAnimationEditors() {
    ...
    return animations.map((animation, index) => {
      const item = animation && typeof animation === "object" && !Array.isArray(animation) ? animation : {};
      return `
      <article class="object-config-card">
        <header>
          <strong>Animation ${index + 1}${item.object_name ? ` · ${this._escape(item.object_name)}` : ""}</strong>
          <button type="button" data-remove-object-item="animation" data-item-index="${index}">Remove</button>
        </header>
        <div class="editor-grid">
          ${this._objectField("animation", index, "object_name", "GLB Object Name", item.object_name, "CeilingFan")}
          ${this._objectField("animation", index, "entity", "Entity", item.entity, "fan.living_room")}
          ${this._objectSelect("animation", index, "type", "Animation Type", item.type || "rotate", [["rotate", "Rotate"], ["oscillate", "Oscillate"], ["bob", "Bob"]])}
          ${this._objectSelect("animation", index, "axis", "Axis", item.axis || "y", [["x", "X"], ["y", "Y"], ["z", "Z"]])}
          ${this._objectField("animation", index, "speed", "Speed", item.speed ?? 1, "1", "number", 'min="0" step="0.1"')}
          ${this._objectField("animation", index, "state_on", "Active State", item.state_on || "on", "on")}
          ${this._objectField("animation", index, "amplitude", "Amplitude", item.amplitude ?? 0.5, "0.5", "number", 'min="0" step="0.1"')}
        </div>
      </article>
    `;
    }).join("");
  ...
  _renderInteractiveObjectEditors() {
    ...
    return objects.map((object, index) => {
      ...
      const tapAction = this._interactiveActionType(item.tap_action, "toggle");
      const holdAction = this._interactiveActionType(item.hold_action, "more-info");
      return `
        <article class="object-config-card">
          <header>
            <strong>Interactive object ${index + 1}${item.object_name ? ` · ${this._escape(item.object_name)}` : ""}</strong>
            <button type="button" data-remove-object-item="interactive" data-item-index="${index}">Remove</button>
          </header>
          <div class="editor-grid">
            ${this._objectField("interactive", index, "object_name", "GLB Object Name", item.object_name, "CeilingFan")}
            ${this._objectField("interactive", index, "entity", "Entity", item.entity, "fan.living_room")}
            ...
            ${(tapAction === "navigate" || holdAction === "navigate")
              ? this._objectField("interactive", index, "navigation_path", "Navigation Path", item.navigation_path || item.navigationPath, "#living-room or /lovelace/room")
              : ""}
          </div>
          ${this._renderServiceActionFields(index, "tap_action", item.tap_action)}
          ${this._renderServiceActionFields(index, "hold_action", item.hold_action)}
          ${this._renderStateStyleEditor(index, item.state_styles)}
        </article>
      `;
    }).join("");
  }
  ...
  _removeObjectConfigItem(kind, index) {
    ...
    const target = this._objectConfigTarget(nextConfig);
    const key = this._objectConfigArrayKey(kind);
    if (!Array.isArray(target[key]) || index < 0 || index >= target[key].length) return;
